...
class Board:
    ...
    def clear(self, rows):
        self.data = np.insert(np.delete(self.data, list(rows), 0), 0,
                              np.zeros((len(rows), self.cols()), dtype=np.bool_), axis=0)
    ...
    def __str__(self):
        return '\n'.join(
            [''.join([self[i, j] if self[i, j] else self[i, j] for j in range(0, self.cols())]) for i in range(0, self.rows())])
